 ...
 */
export function usageCheck(counterType: UsageCounterType, increment: boolean = true) {
    ...
    try {
      ...
      let counter = await db.query.usageCounters.findFirst({
        where: and(
          ...
          eq(usageCounters.type, counterType),
          sql`${usageCounters.periodStart} <= ${now}`,
          sql`${usageCounters.periodEnd} >= ${now}`
        ),
      });
      ...
      if (counter.used >= counter.limit) {
        return next(
          new AppError(
            429,
            `Лимит операций «${counterType}» исчерпан (${counter.used}/${counter.limit}). Обновите подписку для увеличения лимита.`,
      ...
      (req as Record<string, unknown>).usageCounter = {
        id: counter.id,
        used: counter.used,
        limit: counter.limit,
        type: counterType,
...
export async function getUsageStatus(workspaceId: string) {
  ...
  const counters = await db
    ...
    .where(
      and(
        eq(usageCounters.workspaceId, workspaceId),
        sql`${usageCounters.periodEnd} >= ${new Date()}`
      )
    );

  const result: Record<string, { used: number; limit: number; periodEnd: Date | null }> = {};
