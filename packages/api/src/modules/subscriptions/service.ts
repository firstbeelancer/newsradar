...
interface YookassaPaymentResponse {
  id: string;
  status: "pending" | "succeeded" | "cancelled" | "waiting_for_capture";
  amount: { value: string; currency: string };
  confirmation?: {
...
export async function createPayment(input: CreatePaymentInput) {
  ...
  const body: YookassaPaymentRequest = {
    amount: {
 ...
 */
export async function downgradeNow(workspaceId: string) {
  const workspace = await db.query.workspaces.findFirst({
    where: eq(workspaces.id, workspaceId),
  });
  if (!workspace) {
 ...
 */
export async function handleWebhook(payload: YookassaWebhookPayload) {
  ...
  switch (event) {
    case "payment.succeeded": {
      ...
      const plan = (object.metadata?.plan as "monthly" | "yearly") ?? "monthly";
      const now = new Date();
      const periodEnd =
        plan === "yearly"
  ...
  const expiredWorkspaces = await db
    ...
    .where(
      and(
        ...
        lte(workspaces.periodEnd, now)
      )
    );

  const results: Array<{ workspaceId: string; plan: string }> = [];
