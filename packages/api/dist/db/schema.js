import { pgTable, uuid, varchar, text, timestamp, jsonb, boolean, integer, decimal, uniqueIndex, index, check, } from "drizzle-orm/pg-core";
...
export const articleScores = pgTable("article_scores", {
    id: uuid("id").primaryKey().defaultRandom(),
...
export const chipFilters = pgTable("chip_filters", {
    id: uuid("id").primaryKey().defaultRandom(),
    agentId: uuid("agent_id")
        .references(() => agents.id, { onDelete: "cascade" })
        .notNull(),
...
export const articleFingerprints = pgTable("article_fingerprints", {
    id: uuid("id").primaryKey().defaultRandom(),
    articleId: uuid("article_id")
        .references(() => articles.id, { onDelete: "cascade" })
        .notNull(),
...
// ─────────────────────────────────────────────────────────────
export const fetchSchedules = pgTable("fetch_schedules", {
    id: uuid("id").primaryKey().defaultRandom(),
    workspaceId: uuid("workspace_id")
        .references(() => workspaces.id, { onDelete: "cascade" })
        .notNull(),
    ...
    check("fetch_schedules_preset_check", sql `${table.preset} IN ('every_hour', 'every_6h', 'every_day', 'custom')`),
]);
// ─────────────────────────────────────────────────────────────
// Layer 9 — DeepSearch Results
// ─────────────────────────────────────────────────────────────
export const deepsearchResults = pgTable("deepsearch_results", {
    id: uuid("id").primaryKey().defaultRandom(),
    workspaceId: uuid("workspace_id")
        .references(() => workspaces.id, { onDelete: "cascade" })
        .notNull(),
