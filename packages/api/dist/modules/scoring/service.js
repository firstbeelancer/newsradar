...
export async function getScoringConfig(workspaceId) {
...
export async function updateScoringConfig(workspaceId, weights) {
    const validated = validateWeights(weights);
    weightCache.set(workspaceId, validated);
    return { ...validated, workspaceId };
}
// ─── Recalculate scores ───
export async function recalculateScores(workspaceId, params) {
    ...
    for (const article of articlesToScore) {
        ...
        const overallScore = Math.round((aiRelevance + keywordMatch + freshness + sourceTrust) / 4 * 1000) / 1000;
        const weightedScore = Math.round((aiRelevance * weights.aiRelevance +
...
// ─── Scoring stats ───
export async function getScoringStats(workspaceId) {
    ...
    for (const range of scoreRanges) {
        const countResult = await db
            ...
            .from(articles)
            .where(and(eq(articles.workspaceId, workspaceId), range.label === "unscored"
    ...
    const avgScoreResult = await db
        ...
        .from(articles)
        .where(and(eq(articles.workspaceId, workspaceId), sql `${articles.score} > 0`));
    return {
