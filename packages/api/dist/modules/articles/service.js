...
// ─── CRUD ───
export async function getArticleById(id, workspaceId) {
...
export async function listArticles(filters) {
    const conditions = [eq(articles.workspaceId, filters.workspaceId)];
    if (filters.agentId) {
    ...
    if (filters.search) {
        ...
        const tsQuery = filters.search
            ...
            .map((w) => w + ":*")
            .join(" & ");
        conditions.push(sql `to_tsvector('russian', ${articles.title} || ' ' || COALESCE(${articles.description}, '')) @@ to_tsquery('russian', ${tsQuery})`);
    }
    let query = db
        .select()
        .from(articles)
    ...
    if (filters.cursor) {
        ...
        if (decoded?.sortValue) {
            ...
            conditions.push(sql `(${articles.publishedAt} IS NOT NULL AND ${articles.publishedAt} < ${cursorDate}) OR (${articles.publishedAt} IS NULL AND ${articles.createdAt} < ${cursorDate})`);
            query = db
                .select()
                .from(articles)
                .where(and(...conditions))
...
export async function searchArticles(workspaceId, queryText, params) {
    ...
    if (params.cursor) {
        ...
        if (decoded?.sortValue) {
            conditions.push(sql `${articles.score} < ${decoded.sortValue}`);
            query = db
                .select()
                .from(articles)
                .where(and(...conditions))
...
// ─── Favorites ───
export async function addToFavorite(id, workspaceId) {
...
export async function removeFromFavorite(id, workspaceId) {
    await getArticleById(id, workspaceId);
    const [updated] = await db
        .update(articles)
...
export async function listFavorites(workspaceId, params) {
    return listArticles({
        workspaceId,
        isFavorite: true,
        limit: params.limit,
...
export async function getArticleWithScore(id, workspaceId) {
